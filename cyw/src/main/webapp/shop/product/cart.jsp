...
<%
	...
	ArrayList<CyCartDTO> lists = cyitemshopDao.cartuser(sessionid);
	int total=0;
%>    
...
$(document).ready(function() {
	...
	function sendIt(){
		...
		var url = "<%=cp%>/shop/product/orderMain.jsp?userId=<%=userId%>&totalPrice="+totalPrice;
...
<div id="container">
	...
	<div class="cartTab">
				<ul>
					<li class="on"><a href="./cart.jsp?userId=<%=sessionid%>">담긴상품 <strong>${dto.size() }</strong></a></li>
				</ul>
	</div>
	...
	<%for(CyCartDTO dto:lists){	total+=dto.getItemPrice();%>
 
	<tr class="productRow">
				<td class="alR">
				<input type="checkbox" title="상품선택" class="bskPrdctChk bskPrdctItm"
				name="prdct_id" value="<%=dto.getNum()%>" checked="checked" onclick="itemCheck('${dto.itemPrice }');">
			...
			<div class="itemWrap">
				<div class="imgArea">
					...
					<img src="<%=cp %>/img<%=dto.getSaveFileName()%>"/>
					</a>
				</div><!-- //imgArea -->
		...
		<div class="infoArea">
			...
			<ul>
				...
				<!-- 여기서부턴 반복하면 될듯 -->
					...
					<div class="itemPrice"><span class="price"><strong><fmt:formatNumber value="<%=dto.getItemPrice()%>" pattern=" #,###" /></strong>도토리</span></div>
					...
					<div class="itemDelete">
						...
						// 삭제 링크
							function deleteCheck(num) {
								...
								var f = document.myForm;								
								var link = '<%=cp%>/shop/product/cartDelete.jsp?num='+num+'&userId=<%=userId %>';
						...
						<a href="javascript:deleteCheck('<%=dto.getNum()%>');" 
		...
		<td><span class="price"><strong><fmt:formatNumber value="<%=dto.getItemPrice()%>" pattern=" #,###" /></strong>도토리</span></td>
...
<%} %>
	...
	<div class="cartPriceSum" id="cartPriceSum">
		...
		<span class="price"><strong> <%=total%> </strong> 도토리 </span>
...
<script>
	window.onload = function () {
		...
		if('${sessionScope.userid }' == ""){
			location.href='<%=cp %>/homepage/index.jsp';
			return;
		}
		itemCheck();
	}
