...
<script type="text/javascript">
	...
	function goCart(one) {
		...
		f.action = '../cart.jsp?userId=' + '${session.getUserId() }';
		f.submit();
	...
	function sendIt_Order(num) {
		...
		var f = document.myForm;
		f.action = '../product/order.jsp?userId=' + '${sessionScope.userid }&totalPrice=10&num=' + num +'&buyType=buy';
		f.submit();
...
<div id="contents">
	...
	<div class="prdtImgWrap">
		<img src="<%=cp%>/img<%=dto.getSaveFileName()%>" width="300" height="300" name="img1" />
	...
	<div class="prdtImgList">
		...
		<c:forEach var="dto" items="${lists }">
			<li><a href="javascript:location.href='<%=cp%>/shop/product/listdetail.jsp?num=<%=dto.getType()%>';">
				<img src="<%=cp%>/img<%=dto.getSaveFileName()%>" width="80" alt="<%=cp%>/img<%=dto.getSaveFileName()%>"
				height="80" onmouseover="fimg('<%=cp%>/img<%=dto.getSaveFileName()%>')" />
				</a></li>
		</c:forEach>
		</ul>	
	...
	<div class="prdtCode">
		<dl><dt>상품번호 :</dt><dd><%=dto.getNum()%></dd></dl>
	</div>
	
	<div class="prdtTitWrap">
		<p class="prdtCmnt"></p>
		<h2 class="prdtTit">[<%=dto.getType()%>]&nbsp;<%=dto.getItemName()%></h2>
	</div>
	...
	<a href="#none" class="btnD openLyBtn">비회원 구매</a>
		...
		<div class="lyCont"><br/>
			...
			<div class="able" style="padding:20px; clear:both;">
				...
				<a href="<%=cp%>/homepage/createUser.jsp">
	...
	<div class="prdtInfoWrap">
		<dl><dt>판매가</dt>
			<dd><span class="price"><strong><%=dto.getItemPrice()%></strong>
				<img src="<%=cp%>/img/dotori.png" width="18" height="18" name="도토리" /></span>
	...
	<dl>
		<dt>타입</dt><dd><%=dto.getType()%></dd>
	</dl>
	<dl>
		<dt>컨텐츠 원 제작자</dt><dd>컨텐츠 내부 항목 참조</dd>
	</dl>
	<dl>
		<dt>컨텐츠 배포자</dt><dd>싸이월드</dd>
	</dl>
	<dl>
		<dt>원산지</dt><dd>
	...
	<div class="selectedItem">
		...
		<div class="itemName">
			<h6 style="color:black;">아이템명:&nbsp; <%=dto.getItemName()%></h6></div>
		...
		<div class="itemPrice">
			...
			<strong>
				<font style="font-size:15px;">x <%=dto.getItemPrice()%></font>
			</strong>
			</span>
		</div>
	...
	<div class="buyCount"></div>총주문상품금액&nbsp;
		<span class="price">
			<strong><%=dto.getItemPrice()%></strong>
			<img src="<%=cp%>/img/dotori.png" width="20" height="20" name="도토리" />
		</span>
	...
	<div class="orderBtnWrap btn3 alC">
		...
		onclick="insert(<%=cyitemshopDao.selectcart(sessionid, dto.getNum()) %>,<%=dto.getNum()%>);">장바구니 담기</button>
		...
		<button type="button" class="btnBuy" onclick="sendIt_Order('<%=dto.getNum()%>');">구매하기</button>
		<br/><br/>
		</c:otherwise>
	</c:choose>
