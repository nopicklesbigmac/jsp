...
<script type="text/javascript">
	...
	function search() {
		...
		window.open("user_main.jsp?userId="+userKeyword,userKeyword,
		    ...
		    for (var i = 0; i < opts.length; i++) {
		      if (opts[i].value === val) {
		        ...
		        if('${session.getUserId() }' == ""){
		       	...
		       	window.open("user_main.jsp?userId="+userId,userId,
				"width=1090,height=600,location=no,status=no,scrollbars=no");
		        break;
		      }
		    }
...
<div id="header">
	<div class="headerCont LGN_TP_GUEST clB">
		<h1>
			...
			<a id="logourl" href="../homepage/index.jsp?loginOk=ok">
			...
			<c:if test="${session.getUserId()==null }">
			<a id="logourl" href="../homepage/index.jsp">
				<img id="logo" src="<%=cp %>/resources/images/logo.png" 
				style="padding-top:10px; padding-left:45px;	height:45;"
				alt="싸이월드 메인">
			</a>
	...
	<div id="util">
		...
		<c:choose>
			...
			<li><a href="<%=cp%>/homepage/login.jsp">로그인</a></li>
			</c:when>
			<c:otherwise>
			<li><a href="<%=cp%>/homepage/logout.jsp">로그아웃</a></li>
			</c:otherwise>
		</c:choose>
		<c:choose>
			<c:when test="${sessionScope.userid==null }">
			<li><a href="<%=cp%>/homepage/index.jsp">싸이월드</a></li>
			</c:when>
			<c:otherwise>
			<li><a href="<%=cp%>/homepage/index.jsp?loginOk=ok">싸이월드</a></li>
			</c:otherwise>
		</c:choose>
			<li><a href="<%=cp%>/shop/product/list.jsp?type=all">선물가게</a></li>				
		<c:choose>
			<c:when test="${sessionScope.userid==null }">
			<li><a href="<%=cp%>/homepage/createUser.jsp">회원가입</a></li>
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${sessionScope.userid==null}">
			<li><a href="<%=cp%>/shop/cart.jsp">장바구니</a></li>
			</c:when>
		</c:choose>
		</ul>
	</div>
...
<div id="gnb">
	...
	<ul class="gnb">
		<li class=""><a href="javascript:location.href='../shop/product/list.jsp?type=배경음악';"
			class="gnbDep1">배경음악
			</a></li>
		<li class=""><a href="javascript:location.href='../shop/product/list.jsp?type=스킨';"
			class="gnbDep1">스킨
			</a></li>
		<li class=""><a href="javascript:location.href='../shop/product/list.jsp?type=글꼴';"
			class="gnbDep1">글꼴
			</a></li>
		<li class=""><a href="javascript:location.href='../shop/product/list.jsp?type=미니미';"
			class="gnbDep1">미니미
			</a></li>
		<li class=""><a href="javascript:location.href='../shop/product/list.jsp?type=스토리룸';"
			class="gnbDep1">스토리룸
			</a></li>
